...
public class FilterFrame extends JPanel {
	...
	private ActionListener buttonListener = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			WheelSide wheelSide = inputWheel.getSelectedIndex() != 0 ? WheelSide
					.values()[inputWheel.getSelectedIndex() - 1] : null;
			Transmission transmission = inputTransmission.getSelectedIndex() != 0 ? Transmission
					.values()[inputTransmission.getSelectedIndex() - 1] : null;
			int maxPrice = (int) inputMaxPrice.getValue();
			if (maxPrice == 0)
				maxPrice = -1;
			systemWindow.filter(new FilterData(minSeats, wheelSide,
					transmission, maxPrice));
		}
	};
