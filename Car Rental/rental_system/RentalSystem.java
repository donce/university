...
/**
 * Rental system of cars. System has cars and registered customers. Customer can
 * rent a car for some number of days.
 * 
...
public class RentalSystem implements Serializable {
	...
	public void remove(Car car) {
		...
		if (!cars.remove(car) && !orderedCars.remove(car))
			throw new IllegalArgumentException(
					"Car being removed does not exist.");
	 ...
	 * 
	 * @param id
	 *            id of car the key whose associated value is to be returned
	 ...
	 * 
	 * @param filterData
	 *            filter to be used for choosing cars
	...
	public List<Car> getCars(FilterData filterData) {
		...
		for (int i = 0; i < cars.size(); ++i) {
			Car car = cars.get(i);
			if ((filterData.getMinSeats() == -1 || filterData.getMinSeats() <= car
					.getSeats())
					&& (filterData.getWheelSide() == null || filterData
							.getWheelSide() == car.getWheelSide())
					&& (filterData.getTransmission() == null || filterData
							.getTransmission() == car.getTransmission())
					&& (filterData.getMaxPrice() == -1 || filterData
	...
	public void add(Customer customer) throws InvalidFormDataException {
		...
		if (customer.getNumberA() <= 0)
			throw new InvalidFormDataException("Number",
	...
	public void remove(Customer customer) {
		if (customer == null || !customers.contains(customer)
				|| !customerOrders.containsKey(customer))
			throw new IllegalArgumentException();
	 ...
	 * @param lastName
	 *            Customer last name
	 * @return Customer with specified names if it exists in the system,
	 *         otherwise null
	 */
	...
	public void generateCustomers() {
		String[] firstNames = { "Donatas", "Marius", "Rytis", "Vidmantas",
				"Karolis" };
		String[] lastNames = { "Pavardenis", "Pavardžius", "Pavardienius",
				"Pavadžius", "Pavardenis" };
		for (int i = 0; i < Math.min(firstNames.length, lastNames.length); ++i)
			try {
				add(new Customer(firstNames[i], lastNames[i], "Lithuania",
						"Vilnius", "Naugarduko", 10, 2));
